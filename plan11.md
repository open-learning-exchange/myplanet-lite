Actually, since Moshi is not imported here, and `org.json.JSONObject` IS imported (`import org.json.JSONObject`), it is much easier and safer to use `JSONObject` and `JSONArray` as they are already available. Android's `JSONObject` loads the entire string into memory, but since it's `_all_docs` and only has images, the payload size will be bounded by the total size of base64 images in `editInitialImagePaths`. Usually a post doesn't have hundreds of large images. It's safe enough.

Let's do this: I will write a custom Python script to insert the `fetchExistingVoiceImagesBulk` function into `CreateVoiceActivity` and update `loadEditInitialImages` to call it.
...
replacement = """
    private suspend fun loadEditInitialImages() {
        ...
        val loaded = fetchExistingVoiceImagesBulk(base, editInitialImagePaths)
        if (loaded.isEmpty()) {
            return
        }
    ...
    private suspend fun fetchExistingVoiceImagesBulk(
        ...
        val resourcePaths = LinkedHashMap<String, String>()
...
# I need to find `private suspend fun loadEditInitialImages()`
# and replace it.
