...
) : androidx.recyclerview.widget.ListAdapter<SurveyItemUiModel, SurveyItemsAdapter.SurveyViewHolder>(DiffUtils.itemCallback({ old, new -> old.document.id == new.document.id })) {
...
private class SurveyOutboxAdapter(
    private val onOutboxSelected: (OutboxEntry) -> Unit,
) : androidx.recyclerview.widget.ListAdapter<OutboxEntry, SurveyOutboxAdapter.OutboxViewHolder>(DiffUtils.itemCallback({ old, new -> old.id == new.id })) {
